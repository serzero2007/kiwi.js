...
(function (global, factory) {
    ...
    var IndexedMap = /** @class */ (function () {
         ...
         * Create a copy of this associative array.
         */
        IndexedMap.prototype.map = function (callback) {
            return this.array.map(function (item) { return callback(item); });
        };
        return IndexedMap;
    }());
    /**
    ...
    var Variable = /** @class */ (function () {
        function Variable(name, value, context) {
            if (name === void 0) { name = ""; }
    ...
    var Expression = /** @class */ (function () {
        ...
        Object.defineProperty(Expression.prototype, "value", {
            ...
            get: function () {
                return this.constant +
                    this.terms.map(function (p) { return p.first.value * p.second; })
        ...
        Expression.prototype.toString = function () {
            var result = this.terms.map(function (pair) { return pair.second + "*" + pair.first.toString(); });
            if (this.constant !== 0) {
    ...
    function parseArgs(args) {
        ...
        var _loop_1 = function (i, n) {
            ...
            if (item instanceof Variable) {
                terms.setDefault(item, function () { return 1.0; });
                return "continue";
            }
            if (item instanceof Expression) {
                ...
                var _loop_2 = function (j, k) {
                    ...
                    terms.setDefault(termPair.first, function () { return termPair.second; });
                };
            ...
            if (item instanceof Array) {
                ...
                if (value2 instanceof Variable) {
                    terms.setDefault(value2, function () { return value_1; });
                }
                else if (value2 instanceof Expression) {
                    constant += (value2.constant * value_1);
                    var terms2 = value2.terms;
                    var _loop_3 = function (j, k) {
                        var termPair = terms2.itemAt(j);
                        terms.setDefault(termPair.first, function () { return termPair.second * value_1; });
                    };
                    for (var j = 0, k = terms2.size(); j < k; j++) {
                        _loop_3(j, k);
                    }
    ...
    var Strength = { create: create, clip: clip, required: required, strong: strong, medium: medium, weak: weak };
    ...
    var Constraint = /** @class */ (function () {
        ...
        Constraint.prototype.toString = function () {
            return this.expression.toString() + " "
                + exports.Operator[this.operator]
    ...
    var Solver = /** @class */ (function () {
        function Solver() {
            this.constraints = createCnMap();
            this.rows = createRowMap();
            this.vars = createVarMap();
            this.edits = createEditMap();
        ...
        Solver.prototype.addConstraint = function (constraint) {
            ...
            var _a = this._createRow(constraint), row = _a.row, tag = _a.tag;
        ...
        Solver.prototype.removeConstraint = function (constraint) {
            var cnPair = this.constraints.erase(constraint);
            if (cnPair === undefined) {
                throw new Error("unknown constraint");
            }
            var cnTag = cnPair.second;
            ...
            var marker = cnTag.marker;
            var rowPair = this.rows.erase(marker);
            if (rowPair === undefined) {
                var leaving = this._getMarkerLeavingISymbol(marker);
        ...
        Solver.prototype.addEditVariable = function (variable, strength) {
            var editPair = this.edits.find(variable);
            ...
            this.addConstraint(cn);
            var tag = this.constraints.find(cn).second;
            this.edits.insert(variable, { tag: tag, constraint: cn, constant: 0.0 });
        };
        /**
        ...
        Solver.prototype.removeEditVariable = function (variable) {
            var editPair = this.edits.erase(variable);
            if (editPair === undefined) {
        ...
        Solver.prototype.suggestValue = function (variable, value) {
            ...
            for (var i = 0, n = rows.size(); i < n; ++i) {
                ...
                if (coeff !== 0.0 && row.add(delta * coeff) < 0.0 &&
                    rowPair_1.first.type !== ISymbolType.External) {
        ...
        Solver.prototype.updateVariables = function () {
            for (var i = 0, n = this.vars.size(); i < n; ++i) {
                var pair = this.vars.itemAt(i);
                var rowPair = this.rows.find(pair.second);
         ...
         * @private
         */
        Solver.prototype._getVarISymbol = function (variable) {
            var symbol = this._makeISymbol(ISymbolType.External);
            return this.vars.setDefault(variable, function () { return symbol; }).second;
        };
        /**
        ...
        Solver.prototype._createRow = function (constraint) {
            ...
            for (var i = 0, n = terms.size(); i < n; ++i) {
                ...
                if (!nearZero(termPair.second)) {
                    var symbol = this._getVarISymbol(termPair.first);
                    var basicPair = this.rows.find(symbol);
            ...
            switch (constraint.operator) {
                ...
                case exports.Operator.Ge:
                    {
                        var coeff = constraint.operator === exports.Operator.Le ? 1.0 : -1.0;
                        var slack = this._makeISymbol(ISymbolType.Slack);
                        tag.marker = slack;
                        row.insertISymbol(slack, coeff);
                        if (strength < Strength.required) {
                            var error = this._makeISymbol(ISymbolType.Error);
                            tag.other = error;
                ...
                case exports.Operator.Eq:
                    {
                        if (strength < Strength.required) {
                            var errplus = this._makeISymbol(ISymbolType.Error);
                            var errminus = this._makeISymbol(ISymbolType.Error);
                            tag.marker = errplus;
                            tag.other = errminus;
                            ...
                            var dummy = this._makeISymbol(ISymbolType.Dummy);
                            tag.marker = dummy;
        ...
        Solver.prototype._addWithArtificialVariable = function (row) {
            ...
            var art = this._makeISymbol(ISymbolType.Slack);
            ...
            var pair = this.rows.erase(art);
            if (pair !== undefined) {
                var basicRow = pair.second;
                if (basicRow.isConstant()) {
                    return success;
                }
                var entering = this._anyPivotableISymbol(basicRow);
        ...
        Solver.prototype._optimize = function (objective) {
            while (true) {
                var entering = this._getEnteringISymbol(objective);
                if (entering.type === ISymbolType.Invalid) {
                    return;
                }
                var leaving = this._getLeavingISymbol(entering);
                if (leaving.type === ISymbolType.Invalid) {
                    throw new Error("the objective is unbounded");
                }
                // pivot the entering symbol into the basis
                var row = this.rows.erase(leaving).second;
        ...
        Solver.prototype._dualOptimize = function () {
            ...
            while (infeasible.length !== 0) {
                ...
                if (pair !== undefined && pair.second.constant < 0.0) {
                    var entering = this._getDualEnteringISymbol(pair.second);
                    if (entering.type === ISymbolType.Invalid) {
         ...
         * @private
         */
        Solver.prototype._getEnteringISymbol = function (objective) {
         ...
         * @private
         */
        Solver.prototype._getDualEnteringISymbol = function (row) {
            ...
            for (var i = 0, n = cells.size(); i < n; ++i) {
                ...
                if (c > 0.0 && symbol.type !== ISymbolType.Dummy) {
                    var coeff = this.objective.coefficientFor(symbol);
                    var r = coeff / c;
                    if (r < ratio) {
                        ratio = r;
                        entering = symbol;
         ...
         * @private
         */
        Solver.prototype._getLeavingISymbol = function (entering) {
            var ratio = Number.MAX_VALUE;
         ...
         * @private
         */
        Solver.prototype._getMarkerLeavingISymbol = function (marker) {
        ...
        Solver.prototype._removeMarkerEffects = function (marker, strength) {
            var pair = this.rows.find(marker);
            if (pair !== undefined) {
         ...
         * @private
         */
        Solver.prototype._anyPivotableISymbol = function (row) {
            var cells = row.cells();
            for (var i = 0, n = cells.size(); i < n; ++i) {
                var pair = cells.itemAt(i);
         ...
         * @private
         */
        Solver.prototype._makeISymbol = function (type) {
    ...
    function createRowMap() {
        return createMap(ISymbol.Compare);
    }
    /**
     ...
     */
    var ISymbolType;
    (function (ISymbolType) {
        ISymbolType[ISymbolType["Invalid"] = 0] = "Invalid";
        ISymbolType[ISymbolType["External"] = 1] = "External";
        ISymbolType[ISymbolType["Slack"] = 2] = "Slack";
        ISymbolType[ISymbolType["Error"] = 3] = "Error";
        ISymbolType[ISymbolType["Dummy"] = 4] = "Dummy";
    })(ISymbolType || (ISymbolType = {}));
    /**
     * An internal class representing a symbol in the solver.
     * @private
     */
    var ISymbol = /** @class */ (function () {
        /**
     ...
     * @private
     */
    var INVALID_SYMBOL = new ISymbol(ISymbolType.Invalid, -1);
    /**
    ...
    var Row = /** @class */ (function () {
        ...
        function Row(constant) {
            ...
            this._cellMap = createMap(ISymbol.Compare);
        }
        /**
         ...
         */
        Row.prototype.insertISymbol = function (symbol, coefficient) {
         ...
         */
        Row.prototype.removeISymbol = function (symbol) {
            this._cellMap.erase(symbol);
        };
        /**
